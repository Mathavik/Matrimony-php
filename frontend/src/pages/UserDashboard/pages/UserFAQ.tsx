...
export default function UserFAQ() {
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden py-1">
      {/* Header Section */}
      {/* Header Section */}
      {/* Header Section */}
      <section className="flex flex-col lg:flex-row items-center justify-between px-3 sm:px-8 md:px-12 lg:px-16 py-0 bg-gray-50 border-b border-gray-200">        <div className="max-w-xl text-center lg:text-left order-2 lg:order-1 mt-4 lg:mt-0">
        <h1 className="text-xl sm:text-3xl md:text-4xl font-extrabold text-gray-800 mb-1 sm:mb-2 tracking-tight">            WedAura <span className="text-rose-600">FAQ Center</span>
        </h1>
        <p className="text-gray-600 text-xs sm:text-base mb-2 sm:mb-3 px-1 sm:px-0">
        ...
        <div className="order-1 lg:order-2 flex justify-center w-full lg:w-auto">
          <img
            ...
            className="w-[100px] h-[100px] sm:w-[140px] sm:h-[140px] md:w-[200px] md:h-[200px] lg:w-[280px] lg:h-[280px] object-contain"
          />
        </div>
      </section>
      ...
      {/* Main FAQ Section */}
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6 sm:gap-10 pt-8 sm:pt-10 px-3 sm:px-6 ">
        {/* Sidebar */}
          ...
          <ul className="flex flex-col gap-1.5 sm:gap-3">
            {categories.map((cat) => (
              <li
                ...
                className={`cursor-pointer text-xs sm:text-base font-medium transition-all duration-200 p-2 sm:p-3 rounded-lg flex items-center ${activeCategory === cat.title
                  ? "text-rose-700 bg-rose-50 border-l-4 border-rose-600 font-semibold"
                  : "text-gray-700 hover:bg-gray-100"
                  }`}
              >
                <ChevronRight
                  className={`mr-1.5 sm:mr-2 transition-transform ${activeCategory === cat.title ? "text-rose-600" : "text-gray-400"
                    }`}
                />
                {cat.title}
              </li>
